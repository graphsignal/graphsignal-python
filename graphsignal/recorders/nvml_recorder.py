...
class DeviceUsage:
    def __init__(self):
        ...
        self.clock_sm_max_mhz: int = 0
        self.throttle_reasons: int = 0
        self.pstate: int = -1
    ...
    def __str__(self):
        ...
        return (
            ...
            f'gpu_temp_c={self.gpu_temp_c}, power_usage_w={self.power_usage_w}, power_limit_w={self.power_limit_w}, '
            f'clock_sm_mhz={self.clock_sm_mhz}, clock_mem_mhz={self.clock_mem_mhz}, clock_sm_max_mhz={self.clock_sm_max_mhz}, '
            f'throttle_reasons={self.throttle_reasons}, pstate={self.pstate}, fan_speed_percent={self.fan_speed_percent}, '
...
class NVMLRecorder(BaseRecorder):
    ...
    def setup(self):
        ...
        # Prime device data; device-specific identifiers are emitted as per-signal
        # tags by on_tick(), not as global SDK tags.
        self.take_snapshot()
    ...
    def on_tick(self):
        ...
        for idx, device_usage in enumerate(device_usages):
            ...
            if device_usage.power_limit_w > 0:
                sdk.set_gauge(
                    name='gpu.power.limit', tags=device_tags,
                    value=device_usage.power_limit_w, measurement_ts=now_ns, unit='watts')
            if device_usage.clock_sm_mhz > 0:
                sdk.set_gauge(
                    name='gpu.clock.sm', tags=device_tags,
                    value=device_usage.clock_sm_mhz, measurement_ts=now_ns, unit='megahertz')
            if device_usage.clock_mem_mhz > 0:
                sdk.set_gauge(
                    name='gpu.clock.mem', tags=device_tags,
                    value=device_usage.clock_mem_mhz, measurement_ts=now_ns, unit='megahertz')
            if device_usage.clock_sm_max_mhz > 0:
                sdk.set_gauge(
                    name='gpu.clock.sm.max', tags=device_tags,
                    value=device_usage.clock_sm_max_mhz, measurement_ts=now_ns, unit='megahertz')
            if device_usage.throttle_reasons > 0:
                sdk.set_gauge(
                    name='gpu.throttle_reasons', tags=device_tags,
                    value=device_usage.throttle_reasons, measurement_ts=now_ns)
            if device_usage.pstate >= 0:
                sdk.set_gauge(
                    name='gpu.pstate', tags=device_tags,
                    value=device_usage.pstate, measurement_ts=now_ns)
            ...
            if num_xid_errors > 0:
                ...
                for xid_error_code in device_usage.last_xid_error_codes:
                    graphsignal.sdk.sdk().log_message(
            ...
            if device_usage.device_name:
                resource_attrs['device.name'] = device_usage.device_name
    ...
    def take_snapshot(self) -> List[DeviceUsage]:
        ...
        for idx in self._current_device_idxs:
            ...
            try:
                device_usage.power_limit_w = nvmlDeviceGetPowerManagementLimit(handle) / 1000.0
            except Exception as err:
                _log_nvml_error(err)

            try:
                device_usage.clock_sm_mhz = nvmlDeviceGetClockInfo(handle, NVML_CLOCK_SM)
            except Exception as err:
                _log_nvml_error(err)

            try:
                device_usage.clock_mem_mhz = nvmlDeviceGetClockInfo(handle, NVML_CLOCK_MEM)
            except Exception as err:
                _log_nvml_error(err)

            try:
                device_usage.clock_sm_max_mhz = nvmlDeviceGetMaxClockInfo(handle, NVML_CLOCK_SM)
            except Exception as err:
                _log_nvml_error(err)

            try:
                device_usage.throttle_reasons = nvmlDeviceGetCurrentClocksThrottleReasons(handle)
            except Exception as err:
                _log_nvml_error(err)

            try:
                device_usage.pstate = nvmlDeviceGetPerformanceState(handle)
            except Exception as err:
                _log_nvml_error(err)
