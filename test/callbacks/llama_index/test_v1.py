...
class LlamaIndexCallbackHandlerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_callback(self, mocked_upload_span):
        ...
        node_parsing_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.node_parsing')
        self.assertEqual(node_parsing_span.labels, ['root'])
        ...
        chunking_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.chunking')
        ...
        self.assertEqual(chunking_span.context.root_span_id, node_parsing_span.span_id)

        query_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.query')
        self.assertEqual(query_span.labels, ['root'])
        self.assertEqual(find_tag(query_span, 'component'), 'Memory')
        self.assertEqual(find_tag(query_span, 'ct1'), 'v1')

        retrieve_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.retrieve')
        ...
        self.assertEqual(retrieve_span.context.root_span_id, query_span.span_id)

        embedding_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.embedding')
        ...
        synthesize_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.synthesize')
        ...
        llm_span = find_call_by_operation(mocked_upload_span.call_args_list, 'llama_index.llm')
        ...
        self.assertEqual(find_data_count(llm_span, 'context_str', 'char_count'), 3)
        ...
        self.assertEqual(llm_chain_span.context.root_span_id, query_span.span_id)
        ...
        self.assertEqual(fake_llm_span.context.root_span_id, query_span.span_id)
